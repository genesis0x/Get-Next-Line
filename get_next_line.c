...
/*   Updated: 2022/11/05 19:19:12 by hahadiou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	int						c;
	t_line_buffer			line;
	static t_read_buffer	rb[BUFFER_SIZE];

	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, line.buf, 0) < 0)
		return (0);
	if (!(alloc_read_buffer(&rb[fd])))
		return (0);
	line.buf = 0;
	line.pos = 0;
	line.size = 0;
